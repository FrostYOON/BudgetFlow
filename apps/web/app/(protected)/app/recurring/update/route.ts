...
import { updateRecurringTransaction, type RecurringRepeatUnit, type RecurringTransactionType, type RecurringVisibility } from "@/lib/recurring";
...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    return NextResponse.redirect(
      new URL("/app/recurring?error=recurring_update_failed", request.url),
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      new URL("/app/recurring?toast=recurring_updated", request.url),
    );
  } catch {
    return NextResponse.redirect(
      new URL("/app/recurring?error=recurring_update_failed", request.url),
