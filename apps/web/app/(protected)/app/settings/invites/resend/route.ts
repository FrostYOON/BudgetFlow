...
import { resendWorkspaceInvite } from "@/lib/settings";
...
export async function POST(request: NextRequest) {
  ...
  if (!workspaceId || !inviteId) {
    return NextResponse.redirect(
      new URL("/app/settings?error=invite_resend_failed", request.url),
    );
  }
  ...
  try {
    ...
    return NextResponse.redirect(
      new URL("/app/settings?toast=invite_resent", request.url),
    );
  } catch {
    return NextResponse.redirect(
      new URL("/app/settings?error=invite_resend_failed", request.url),
